...
def _default_dingding_secret_env_var(*, account_id: str, secret_key: str) -> str:
    ...
    key = (
        DEFAULT_GATEWAY_ACCOUNT_ID if account_id == "default" else account_id,
        secret_key,
    )
    if key in defaults:
        return defaults[key]
...
def _dingding_account_secret_env_vars(
    account_payload: Mapping[str, object],
) -> tuple[str, ...]:
    env_payload = _mapping(account_payload.get("env")) or {}
    env_vars: list[str] = []
...
def _discord_account_secret_env_vars(
    account_payload: Mapping[str, object],
) -> tuple[str, ...]:
    env_payload = _mapping(account_payload.get("env")) or {}
...
def _feishu_account_secret_env_vars(
    account_payload: Mapping[str, object],
) -> tuple[str, ...]:
    env_vars: list[str] = []
    env_payload = _mapping(account_payload.get("env")) or {}
...
def _weixin_account_secret_env_vars(
    account_payload: Mapping[str, object],
) -> tuple[str, ...]:
    return ()
...
def _wecom_account_secret_env_vars(
    account_payload: Mapping[str, object],
) -> tuple[str, ...]:
    env_payload = _mapping(account_payload.get("env")) or {}
    env_vars: list[str] = []
...
__all__ = [
    ...
    "_default_dingding_secret_env_var",
    "_find_dingding_account",
    "_resolved_dingding_secret_env_var",
    "_upsert_dingding_account",
    "_dingding_account_secret_env_vars",
    ...
    "_find_weixin_account",
    "_upsert_weixin_account",
    "_weixin_account_secret_env_vars",
    "_find_wecom_account",
    "_upsert_wecom_account",
    "_wecom_account_secret_env_vars",
    "_default_wecom_secret_env_var",
    "_resolved_wecom_secret_env_var",
    "_resolved_defaults",
