...
from apps.runtime_layout import default_gateway_state_dir
    ...
    if accounts_value is None:
        ...
        for index, account in enumerate(accounts_value):
            if not isinstance(account, Mapping):
                raise SystemExit(f"gateway.adapters.discord.accounts[{index}] must be a JSON object")
...
def _run_add_feishu(args: Namespace) -> int:
    ...
    if accounts_value is None:
        ...
        for index, account in enumerate(accounts_value):
            if not isinstance(account, Mapping):
                raise SystemExit(f"gateway.adapters.feishu.accounts[{index}] must be a JSON object")
            existing_accounts.append({str(key): value for key, value in account.items()})
    else:
    ...
    transport = (
        ...
        else str((existing_account or {}).get("surface") or feishu_payload.get("surface") or "long-connection")
    )
    event_path = (
        str(args.event_path)
        if args.event_path is not None
        else str(
            (existing_account or {}).get("event_path") or feishu_payload.get("event_path") or DEFAULT_FEISHU_EVENT_PATH
        )
    )
...
def _run_add_dingding(args: Namespace) -> int:
    ...
    transport = (
        str(args.transport or "").strip()
        or str((existing_account or {}).get("surface") or "").strip()
        or str(dingding_payload.get("surface") or "").strip()
        or "stream"
    )
    client_id_env_var = _resolved_dingding_secret_env_var(
        explicit_env_var=args.client_id_env_var,
        existing_account=existing_account,
        account_id=account_id,
        secret_key="client_id",
    )
    client_secret_env_var = _resolved_dingding_secret_env_var(
        explicit_env_var=args.client_secret_env_var,
        existing_account=existing_account,
        account_id=account_id,
        secret_key="client_secret",
    )
    robot_code_env_var = _resolved_dingding_secret_env_var(
        explicit_env_var=args.robot_code_env_var,
        existing_account=existing_account,
        account_id=account_id,
        secret_key="robot_code",
    )
    ...
    if use_wizard:
        if not _print_gateway_dingding_wizard_intro():
            _print_gateway_setup_paused("DingDing")
            return 0
    ...
    account_payload: dict[str, object] = {
        ...
        "env": {
            ...
            "robot_code": robot_code_env_var,
        },
    }
    local_secrets = {}
    if client_id_value:
        local_secrets[client_id_env_var] = client_id_value
    if client_secret_value:
        local_secrets[client_secret_env_var] = client_secret_value
    if robot_code_value:
        local_secrets[robot_code_env_var] = robot_code_value
    control_payload.pop("default_elephant_id", None)
    control_payload.pop("default_session_id", None)
    control_payload.pop("auto_create_elephant", None)
    if allow_group_chats:
    ...
    if control_payload:
        dingding_payload["control"] = control_payload
    else:
        dingding_payload.pop("control", None)
    ...
    if auto_start:
        ...
        try:
            _start_dingding_runtime_after_setup(args, transport=transport)
        except SystemExit:
            print("- Start it again with `elephant gateway dingding start --detach`.")
            return 1
        print("DingDing setup is complete.")
        return 0
...
def _run_remove_dingding(args: Namespace) -> int:
    ...
    if remaining_accounts:
        dingding_payload["accounts"] = remaining_accounts
        dingding_payload["enabled"] = True
        adapters_payload["dingding"] = dingding_payload
    else:
        adapters_payload.pop("dingding", None)
    if adapters_payload:
        gateway_payload["adapters"] = adapters_payload
        manifest["gateway"] = gateway_payload
    else:
...
def _run_add_weixin(args: Namespace) -> int:
    ...
    if control_payload:
        weixin_payload["control"] = control_payload
    else:
        weixin_payload.pop("control", None)
    ...
    print("WeChat transport: ilink")
    if auto_start:
        print("Starting the configured WeChat bridge in the background...")
        try:
            _start_weixin_runtime_after_setup(args, transport="ilink")
        except SystemExit:
            print("- Start it again with `elephant gateway weixin start --detach`.")
            return 1
        print("WeChat setup is complete.")
        return 0
...
def _run_remove_weixin(args: Namespace) -> int:
    ...
    if remaining_accounts:
        weixin_payload["accounts"] = remaining_accounts
        weixin_payload["enabled"] = True
        adapters_payload["weixin"] = weixin_payload
    else:
        adapters_payload.pop("weixin", None)
    if adapters_payload:
        gateway_payload["adapters"] = adapters_payload
        manifest["gateway"] = gateway_payload
    else:
...
def _run_add_wecom(args: Namespace) -> int:
    ...
    transport = (
        str(args.transport or "").strip()
        or str((existing_account or {}).get("surface") or "").strip()
        or str(wecom_payload.get("surface") or "").strip()
        or "websocket"
    )
    bot_id_env_var = _resolved_wecom_secret_env_var(
        explicit_env_var=args.bot_id_env_var,
        existing_account=existing_account,
        account_id=account_id,
        secret_key="bot_id",
    )
    secret_env_var = _resolved_wecom_secret_env_var(
        explicit_env_var=args.secret_env_var,
        existing_account=existing_account,
        account_id=account_id,
        secret_key="secret",
    )
    ...
    if use_wizard:
        if not _print_gateway_wecom_wizard_intro():
            _print_gateway_setup_paused("WeCom")
            return 0
        ...
        if ws is None:
            _print_gateway_setup_paused("WeCom")
            return 0
        account_id, transport, bot_id_value, secret_value = (
            ws.account_id,
            ws.transport,
            ws.bot_id_value,
            ws.secret_value,
    ...
    account_payload: dict[str, object] = {
        ...
        "surface": transport,
        "enabled": True,
        "env": {"bot_id": bot_id_env_var, "secret": secret_env_var},
    }
    local_secrets = {}
    if bot_id_value:
        local_secrets[bot_id_env_var] = bot_id_value
    if secret_value:
        local_secrets[secret_env_var] = secret_value
    control_payload.pop("default_elephant_id", None)
    control_payload.pop("default_session_id", None)
    control_payload.pop("auto_create_elephant", None)
    if allow_group_chats:
    ...
    if control_payload:
        wecom_payload["control"] = control_payload
    else:
        wecom_payload.pop("control", None)
    ...
    if auto_start:
        ...
        try:
            _start_wecom_runtime_after_setup(args, transport=transport)
        except SystemExit:
            print("- Start it again with `elephant gateway wecom start --detach`.")
            return 1
        print("WeCom setup is complete.")
        return 0
...
def _run_remove_wecom(args: Namespace) -> int:
    ...
    if remaining_accounts:
        wecom_payload["accounts"] = remaining_accounts
        wecom_payload["enabled"] = True
        adapters_payload["wecom"] = wecom_payload
    else:
        adapters_payload.pop("wecom", None)
    if adapters_payload:
        gateway_payload["adapters"] = adapters_payload
        manifest["gateway"] = gateway_payload
    else:
...
__all__ = [
    "_run_add_discord",
    "_start_discord_runtime_after_setup",
    "_start_feishu_runtime_after_setup",
    "_run_add_feishu",
    "_run_remove_discord",
    "_run_remove_feishu",
    "_start_dingding_runtime_after_setup",
    "_run_add_dingding",
    "_run_remove_dingding",
    "_start_weixin_runtime_after_setup",
    "_run_add_weixin",
    "_run_remove_weixin",
    "_run_add_wecom",
    "_run_remove_wecom",
