...
@unittest.skipUnless(
    _stack_available(),
    "Monitoring stack not running (need Jaeger + Prometheus on localhost)",
)
class ObservabilityE2ETest(unittest.TestCase):
    ...
    def test_traces_metrics_and_logs(self) -> None:
        ...
        self.assertTrue(
            any("invoke_agent" in op for op in operations),
            f"No invoke_agent span: {operations}",
        )
        self.assertTrue(any("chat" in op for op in operations), f"No chat span: {operations}")
        self.assertTrue(
            any("execute_tool" in op for op in operations),
            f"No execute_tool span: {operations}",
        )
        ...
        print(
            f"  chat span: model={tags.get('gen_ai.request.model')}, "
        ...
        self.assertTrue(
            all(l.get("trace_id") for l in episode_logs),
            "Some entries missing trace_id",
        )
        print(f"  Total entries: {len(log_lines)}, with episode_id: {len(episode_logs)}")

        for entry in log_lines:
            print(
                f"  [{entry.get('level')}] trace={entry.get('trace_id', '')[:8]} "
