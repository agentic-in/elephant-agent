...
class DesignClosureContractsTest(unittest.TestCase):
    ...
    def test_macos_voice_capture_ignores_stale_permission_callbacks(self) -> None:
        ...
        self.assertIn("isPreparingCapture", speech_input)
        self.assertIn("guard !isRecording && !isTranscribing && !isPreparingCapture else { return }", speech_input)
        ...
        self.assertIn("audioApplicationRecordPermissionAllowsCapture", speech_input)
        self.assertIn("AVAudioApplication.shared.recordPermission", speech_input)
        self.assertNotIn("AVAudioApplication.requestRecordPermission", speech_input)
        self.assertIn("requestCaptureDeviceMicrophoneAccess", speech_input)
        self.assertIn("AVCaptureDevice.authorizationStatus(for: .audio)", speech_input)
        self.assertIn("AVCaptureDevice.requestAccess(for: .audio)", speech_input)
        self.assertIn("completionGate.deliver(allowed, completion: completion)", speech_input)
        self.assertIn("self.isPreparingCapture = false", speech_input)
        self.assertIn("self.onText = nil", speech_input)
        self.assertIn("self.captureGeneration += 1", speech_input)
        ...
        self.assertIn("LocalPermissionsSettingsContent()", views)
        self.assertIn("MacPrivacySettings.openFilesAndFolders()", views)
        self.assertIn("MacPrivacySettings.openAutomation()", views)
        ...
        self.assertIn("x-apple.systempreferences:com.apple.preference.security?Privacy_FilesAndFolders", views)
        self.assertIn("x-apple.systempreferences:com.apple.preference.security?Privacy_Automation", views)
        self.assertIn("speech.isTranscribing || speech.isPreparingCapture", views)
        self.assertIn('en: "Preparing voice input"', views)
    ...
    def test_macos_ad_hoc_build_keeps_audio_permission_entitlements(self) -> None:
        ...
        self.assertIn("MACOS_SIGNING_IDENTITY ?= -", (ROOT / "Makefile").read_text(encoding="utf-8"))
        self.assertIn("com.apple.security.device.audio-input", (ROOT / "apps" / "macos" / "Entitlements.plist").read_text(encoding="utf-8"))
        self.assertIn("Local builds default to `MACOS_SIGNING_IDENTITY=-`", readme)
