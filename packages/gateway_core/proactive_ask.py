...
def _personal_model_id(app: Any, record: Any) -> str | None:
    if record.state_id:
        try:
            ...
            LOGGER.debug(
                "Failed to load state %s while resolving proactive ask identity.",
                record.state_id,
                exc_info=True,
            )
            state = None
...
def _timezone_offset(repository: Any, personal_model_id: str, *, now: datetime) -> int:
    ...
    if tz_name:
        try:
            ...
            LOGGER.debug("Ignoring unknown proactive ask timezone %s.", tz_name)
        except Exception:
            LOGGER.debug("Failed to resolve proactive ask timezone %s.", tz_name, exc_info=True)
...
def _profile_timezone_name(repository: Any, personal_model_id: str) -> str | None:
    ...
    try:
        ...
        LOGGER.debug(
            "Failed to load personal model runtime state %s while resolving proactive ask timezone.",
            personal_model_id,
            exc_info=True,
        )
        return _env_tz()
