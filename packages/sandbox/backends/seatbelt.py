...
class SeatbeltPolicyBuilder:
    ...
    def add_home_dotfiles_read(self, home: Path) -> None:
        """Allow reading common home dotfiles needed by dev tools.

        Permits git config, shell configs, etc. but NOT credential files
        (those are blocked by the credential deny rules which take priority).
        """
        # Individual files that tools commonly need
        _DOTFILES = (
            ".gitconfig", ".gitignore_global", ".git-credentials",
            ".curlrc", ".wgetrc", ".inputrc", ".editorconfig",
        )
        # Directories that tools need (non-credential)
        _DOTDIRS = (
            ".config",   # XDG config (git, npm, pip, etc.)
            ".npm",      # npm cache/config
            ".gem",      # ruby gems
            ".m2",       # maven
            ".gradle",   # gradle
        )
        lines: list[str] = ["; --- Home dotfiles (non-credential) ---"]
        for fname in _DOTFILES:
            fpath = home / fname
            if fpath.exists():
                param_key = f"DOTFILE_{fname.lstrip('.').upper().replace('-', '_')}"
                self._params[param_key] = str(fpath.resolve())
                lines.append(f'(allow file-read* (literal (param "{param_key}")))')
        for dname in _DOTDIRS:
            dpath = home / dname
            if dpath.exists():
                param_key = f"DOTDIR_{dname.lstrip('.').upper()}"
                self._params[param_key] = str(dpath.resolve())
                lines.append(f'(allow file-read* file-test-existence (subpath (param "{param_key}")))')
        if len(lines) > 1:  # more than just the comment
            self._extra_policy_lines.extend(lines)
...
class SeatbeltBackend:
    ...
    def run_command(
        ...
        *,
        ...
        # Record violations to persistent store
        if any(d.startswith("sandbox:denied") for d in diagnostics):
            self._record_violations(tuple(diagnostics), command)
    ...
    def _build_policy(self, cwd: Path, sandbox_root: Path) -> tuple[str, list[str]]:
        ...
        if seatbelt_opts.restrict_file_read:
            ...
            builder.add_home_dotfiles_read(Path.home())
    ...
    def _record_violations(self, diagnostics: tuple[str, ...], command: str) -> None:
        """Persist violations to the store for later review."""
        try:
            from ..violations import ViolationStore
            from pathlib import Path

            # Determine state dir from config or use default
            state_dir = Path.home() / ".elephant"
            store = ViolationStore(state_dir)
            store.record_from_diagnostics(diagnostics, command=command)
        except Exception:
            pass  # Never fail the command due to violation recording
