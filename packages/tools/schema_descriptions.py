...
_PROPERTY_DESCRIPTIONS: Mapping[str, Mapping[str, str]] = {
    ...
    "tool.paths.manage": {
        "action": "Use create_path/delete_path for directions, create_baby to define a specialist baby, create_step/update_step/delete_step for Flow items, create_run/update_run/retry_run for baby execution, write_summary after work, and check_understanding when the user confirms learning.",
        "path_id": "Durable Path id returned by create_path or list.",
        "path_step_id": "Flow step id returned by create_step, list, or path inspection.",
        "run_id": "Flow run id returned by create_run or list; pass it to update_run, retry_run, or write_summary.",
        "summary_id": "Learning summary id returned by write_summary.",
        "title": "Short user-facing title.",
        "description": "Plain-language context for the Path or Flow step.",
        "status": "Path status, Flow step status, or run status depending on action.",
        "assignee_elephant_id": "Baby elephant id responsible for a Flow step.",
        "runtime_id": "Runtime or local computer handling a Flow run.",
        "session_id": "Execution session id for the baby elephant run.",
        "work_dir": "Workspace directory used by the run when relevant.",
        "progress_stage": "Short current stage for a run, such as queued, planning, running, summary, or retrying.",
        "progress_detail": "Human-readable progress detail for a run.",
        "progress_current": "Completed progress units when known.",
        "progress_total": "Total progress units when known.",
        "failure_reason": "Reason recorded when a run fails or is cancelled.",
        "reason": "Reason for retry_run.",
        "steps": "Optional initial Flow steps when creating a Path.",
        "what_done": "Concise model-compressed summary of the core result produced by the baby elephant.",
        "why_it_matters": "Why this result matters for the user's direction.",
        "how_it_was_done": "Operational explanation of the approach.",
        "knowledge": "Concepts, tools, or domain knowledge the user should absorb.",
        "human_takeaway": "Shortest checkable learning takeaway the user should absorb.",
        "note": "Human note for an understanding check.",
    },
    "tool.message.send": {
